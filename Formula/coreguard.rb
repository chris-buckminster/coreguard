class Coreguard < Formula
  ...
  def post_install
    (etc/"coreguard").mkpath
    (etc/"coreguard/blocklists").mkpath

    touch etc/"coreguard/custom-allow.txt" unless (etc/"coreguard/custom-allow.txt").exist?
    touch etc/"coreguard/custom-block.txt" unless (etc/"coreguard/custom-block.txt").exist?
  end

  def caveats
    <<~EOS
      To start coreguard as a DNS blocker, you need root privileges:

        sudo coreguard install

      This installs a launchd service that starts coreguard on boot.

      Alternatively, run manually:

        sudo coreguard start

      To manage the Homebrew service (runs as root):

        sudo brew services start coreguard

      Config directory: #{etc}/coreguard/
    EOS
  end

  service do
    run [opt_bin/"coreguard", "start", "--foreground"]
    keep_alive true
    require_root true
    log_path var/"log/coreguard.log"
    error_log_path var/"log/coreguard.log"
  end

  test do
    assert_match version.to_s, shell_output("#{bin}/coreguard --version")
  end
end
